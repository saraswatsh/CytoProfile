...
#' results <- cyt.skku(cytodata[, -c(1, 4)],
#'   printResLog = TRUE,
#'   group.cols = c("Group", "Treatment")
#' ) # Extracting values
#' pdf("barErrorPlot.pdf")
#' par(mfrow = c(2, 2), mar = c(8.1, 4.1, 4.1, 2.1))
#' for (k in 1:nCytokine) {
#'   data <- data.frame("name" = rownames(results[, , k]), results[, , k])
#'   cyt.errbp(data,
#'     pLab = FALSE, esLab = FALSE, classSymbol = TRUE,
...
cyt.errbp <- function(data, pLab = TRUE, esLab = TRUE, classSymbol = TRUE,
  ...
  pvalue <- data[-1, "p.value"]
  effect.size <- data[-1, "effect.size"]
  ...
  if (pLab) {
    if (classSymbol) {
      text(
        x = mid[-1], y = Center[-1] + sign(Center[-1]) * (Spread[-1] + yRange / 20),
  ...
  if (esLab) {
    if (classSymbol) {
      text(
        x = mid[-1], y = Center[-1] + sign(Center[-1]) * (Spread[-1] + yRange / 8),
