...
cyt.plsda <- function(data, group.col = NULL, trt.col = NULL, colors = NULL, title,
  ...
  if (group.col == trt.col) {
    ...
    cytokine.splsda <- mixOmics::splsda(theData.df, theGroups,
      scale = TRUE, ncomp = comp.num,
    ...
    plot_args <- list(cytokine.splsda,
    ...
    if (roc) {
      ...
      roc_obj <- auroc(
        object = cytokine.splsda, newdata = theData.df, outcome.test = theGroups,
    ...
    if (!is.null(cv.opt)) {
      if (cv.opt == "loocv") {
        ...
        a <- ggplot(error_df, aes(x = Component, y = ErrorRate, color = Distance, group = 1)) +
          ...
          geom_point(size = 3) +
          labs(
            title = paste("LOOCV Error Rate:", Title),
        ...
        a <- ggplot(error_df, aes(x = Component, y = ErrorRate, color = Distance, group = 1)) +
          ...
          geom_point(size = 3) +
          labs(
            title = paste("Mfold Error Rate:", Title),
            x = "Number of Components",
            y = "Error Rate"
          ) +
          theme_minimal() +
    ...
    cytokine.splsda2 <- mixOmics::splsda(theData.mat, theGroups,
      scale = TRUE, ncomp = comp.num,
    ...
    plot_args2 <- list(cytokine.splsda2,
      ind.names = NA, legend = TRUE, col = colors,
      pch = pch.values, pch.levels = group_factors,
    ...
    if (!is.null(cv.opt)) {
      if (cv.opt == "loocv") {
        ...
        a <- ggplot(error_df2, aes(x = Component, y = ErrorRate, color = Distance, group = 1)) +
          ...
          geom_point(size = 3) +
          labs(
            title = paste("LOOCV Error Rate (VIP>1):", Title),
            x = "Number of Components",
            y = "Error Rate"
          ) +
          theme_minimal() +
        ...
        a <- ggplot(error_df2, aes(x = Component, y = ErrorRate, color = Distance, group = 1)) +
          ...
          geom_point(size = 3) +
          labs(
            title = paste("Mfold Error Rate (VIP>1):", Title),
            x = "Number of Components",
            y = "Error Rate"
          ) +
          theme_minimal() +
    ...
    if (roc) {
      roc_obj2 <- auroc(
        object = cytokine.splsda2, newdata = theData.mat, outcome.test = theGroups, plot = TRUE,
