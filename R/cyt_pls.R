...
#' @examples
#' cyt_plsda(data.df, colors = c("black", "purple"), title = "Example Analysis.pdf", bg = TRUE, conf.mat = TRUE)
#' cyt_plsda(data.df, colors = c("black", "purple"), title = "Example Analysis.pdf", ellipse = TRUE, conf.mat = TRUE)
#' cyt_plsda(data.df, colors = c("black", "purple"), title = "Example Analysis.pdf", conf.mat = FALSE)
...
cyt_plsda = function(x.df, colors = NULL, title, ellipse = FALSE, bg = FALSE, conf.mat = FALSE){
  ...
  for(i in 1:length(Stimulation.vec) ) {
    ...
    theGroup  = x.df[condt, "group"]
    cytokine.splsda =
      splsda(theData.df, theGroup, scale=TRUE, ncomp=2, keepX = c(ncol(x.df)-2, ncol(x.df)-2))

    splsda.predict = predict( cytokine.splsda, theData.df, dist="max.dist")
    Prediction1 = cbind( original = theGroup, splsda.predict$class$max.dist )
    ...
    cytokine.splsda =
      splsda(theData.mat, theGroup, scale=TRUE, ncomp=2, keepX = c(KeepX, KeepX))


    splsda.predict = predict( cytokine.splsda, theData.mat, dist="max.dist")
    Prediction2 = cbind( original = theGroup, splsda.predict$class$max.dist )
