...
#'
#' @param data A matrix or data frame containing the raw data. If `group.cols` is specified, the columns with names in `group.cols` are treated as grouping variables and all other columns are assumed to be numeric measurement variables.
...
cyt.skku <- function(data, group.cols = NULL, Title = NULL,
  ...
  if (!is.null(group.cols)) {
    measure.cols <- setdiff(names(data), group.cols)
  } else {
    measure.cols <- names(data)
  }

  # Calculate a cutoff for log2 transformation:
  measure.mat <- data[, measure.cols, drop = FALSE]
  ...
  if (!is.null(group.cols)) {
    ...
    treatment_levels <- unique(data[[treatment.col]])

    # Loop over each treatment level.
    for (trt in treatment_levels) {
      subset_df <- data[data[[treatment.col]] == trt, ]
